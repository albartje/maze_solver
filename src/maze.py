...
class Maze:
    ...
    def _solve_r(self,i,j):
        self._animate()
        self._cells[i][j].visited = True
        if i == self._num_rows and j == self._num_cols:
            return True        

        self.visited_path_i.append(i)
        self.visited_path_j.append(j)

        while not self._cells[self._num_cols-1][self._num_rows-1].visited:
